...
Load from `reference/docs.md` when 형식 = .docx. 본문 문서(보고서·제안서·학습지·공문). 슬라이드는 SLIDES, 인쇄 포스터는 POSTER.
...
doc = Document()                                   # 또는 Document("template.docx")로 회사 양식 상속
...
OS별 안전 폰트명은 `templates/doc-env.py`(`korean_docx_font_name()`)에 위임 - Windows `Malgun Gothic`, macOS `AppleGothic`/`Apple SD Gothic Neo`, 공통 배포는 `Nanum Gothic`. 폰트는 파일이 아니라 **이름**으로만 들어가므로, 여는 쪽 PC에 그 폰트가 있어야 동일하게 보인다 - 협업 문서는 범용 폰트(맑은 고딕/나눔)를 권장.
...
업무 문서 -> `templates/doc-gate.sh <vault> report.docx`. korean-gate는 텍스트(.md/.txt) 대상이므로, 본문을 `doc-claims.md`에 근거로 옮겨 적거나 별도 .txt로 내보내 검사한다(이미지화된 docx 자체는 텍스트 스캔 불가). 수치·날짜는 `facts.json` 출처 필수. 교육 대상 학습지면 `edu-gate.sh`.
