#!/usr/bin/env bash
# doc-gate - 업무/성인 대상 문서 preflight. Exit 0 = ship-ready; 1 = violation; 2 = usage.
# 교육 게이트(curriculum/read-level)는 의도적으로 제외 - 학생 배포 문서는 edu-gate.sh를 쓴다.
# "vault" = 문서 한 건의 작업 디렉토리. 반드시 포함:
#   doc-claims.md         (형식/대상/변환 타깃 선언; 교육 dials 없음 = doc 트랙 신호)
#   facts.json            (수치/날짜/통계 출처, [] 허용 -> integrity-gate)
# 선택:
#   contrast-pairs.json   (표지/도표 색을 declare했을 때 -> contrast-gate)
# 검사 대상 = 본문 텍스트 근거(.md/.txt/.html). 바이너리 문서(.docx/.pdf/.hwpx)는
#   export_text 등으로 텍스트를 뽑아 vault에 두고 그 파일을 검사한다(바이너리 직접 스캔 금지).
...
fail() { echo "doc-gate: $*" >&2; echo "== DOC PREFLIGHT: FAIL =="; exit 1; }
usage() { echo "usage: doc-gate.sh <vault> [<text source files>]" >&2; exit 2; }
...
# 1. doc-claims.md (교육 dials 없음 - edu-gate와의 핵심 차이).
CLAIMS="$VAULT/doc-claims.md"
[ -f "$CLAIMS" ] || fail "doc-claims.md missing (형식/대상/변환 타깃을 선언). 교육 대상 문서면 edu-gate.sh를 쓸 것"
...
TXT=()
for f in "${SRC[@]:-}"; do case "$f" in *.md|*.txt|*.html) [ -f "$f" ] && TXT+=("$f");; esac; done
[ "${#TXT[@]}" -gt 0 ] || fail "검사할 텍스트 근거 없음 - 본문을 .txt/.md로 추출해 vault에 두고 재실행 (바이너리 문서는 export_text/pdfplumber 등)"
...
echo "== DOC PREFLIGHT: PASS == (${#TXT[@]} text source file(s))"
exit 0
