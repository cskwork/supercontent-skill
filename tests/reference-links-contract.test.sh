#!/usr/bin/env bash
# /supercontent - reference-link contract. Every reference/*.md path mentioned in
# SKILL.md must exist on disk. A dangling link in SKILL.md sends a reader (or a
# routed builder) to a file that is not there. Each case asserts BOTH an exit code
